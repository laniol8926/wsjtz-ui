 ...
 *
 * Vendored from https://github.com/stephenhouser/node-red-contrib-wsjt-x
 * (packages/node-red-contrib-wsjt-x/wsjt-x-parser.js, v2.0.1) for use
 * outside Node-RED. Field layouts cross-checked against WSJT-Z 2.0.18's own
 * Network/NetworkMessage.hpp and confirmed matching (Status/Decode/Reply/
 * HaltTx/FreeText/QSOLogged) -- schema_number 3 / DEFAULT_SCHEMA 3 agree.
 * Two fixes applied, both confirmed against the real protocol doc
 * (wsjt-z-2.0.18's Network/NetworkMessage.hpp):
 *   1. statusParser's `freqency` field renamed to `frequency` (typo in the
 *      original).
 *   2. replyModifier's bit values corrected -- the original had
 *      shift/ctrl/alt off by one bit (0x01/0x02/0x04) versus the documented
 *      0x02/0x04/0x08. windows/keypad/group (0x10/0x20/0x40) were already
 *      correct. Only matters if a Reply is ever sent with a non-zero
 *      modifier; modifiers=0 ("no modifier", a plain double-click) is
...
class WSJTXParser {
	...
	replyModifier = {
		shift: 0x02,	// shift
		ctrl: 0x04,		// ctrl or CMD
		alt: 0x08,		// ALT
		windows: 0x10,	// windows key on windows (META)
